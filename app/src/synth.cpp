...
static const char Fromsynth[] = "synth";

// See: http://www.deimos.ca/notefreqs/
const float Csynth::s_KeyFrequency[/* MIDI key number */] =
{
...
Csynth *Csynth::s_pThis = 0;

Csynth::Csynth (CInterruptSystem *pInterrupt, CI2CMaster *pI2CMaster)
#ifdef USE_USB
...
boolean Csynth::Initialize (void)
{
	CLogger::Get ()->Write (Fromsynth, LogNotice,
...
void Csynth::Process (boolean bPlugAndPlayUpdated)
{
...
unsigned Csynth::GetChunk (s16 *pBuffer, unsigned nChunkSize)
{
...
unsigned Csynth::GetChunk (u32 *pBuffer, unsigned nChunkSize)
{
	unsigned nChannels = GetHWTXChannels ();
	unsigned nResult = nChunkSize;
...
void Csynth::MIDIPacketHandler (unsigned nCable, u8 *pPacket, unsigned nLength)
{
	assert (s_pThis != 0);
	...
	if (ucType == MIDI_NOTE_ON)
	{
		...
		if (s_pThis->nActiveNotes < NUM_VOICES) {
			...
			for (int i = 0; i < NUM_VOICES; i++) {
				if (s_pThis->noteOrder[i] == 0 || s_pThis->notes[i] == 0) {
					s_pThis->noteOrder[i] = NUM_VOICES;
					s_pThis->notes[i] = ucKeyNumber;
					noteIdx = i;
				}
				s_pThis->noteOrder[i]--;
		...
		s_pThis->voices[noteIdx].NoteOn(ucKeyNumber, ucVelocity);
	}
...
void Csynth::KeyStatusHandlerRaw (unsigned char ucModifiers, const unsigned char RawKeys[6])
{
	assert (s_pThis != 0);
...
void Csynth::USBDeviceRemovedHandler (CDevice *pDevice, void *pContext)
{
	assert (s_pThis != 0);

	if (s_pThis->m_pMIDIDevice == (CUSBMIDIDevice *) pDevice)
	{
		CLogger::Get ()->Write (Fromsynth, LogDebug, "USB MIDI keyboard removed");

		s_pThis->m_pMIDIDevice = 0;
	}
	else if (s_pThis->m_pKeyboard == (CUSBKeyboardDevice *) pDevice)
	{
		CLogger::Get ()->Write (Fromsynth, LogDebug, "USB PC keyboard removed");
